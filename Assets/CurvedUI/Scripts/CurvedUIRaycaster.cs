...
#region PHYSICS RAYCASTING
        ...
        public bool PointToCylinderRadius(Vector3 point, out float distance, out Ray outRay, out Vector2 pointOnCanvas)
        {
            RaycastHit hit = new RaycastHit();
            Vector3 cyllinderMidPointWorld = myCanvas.transform.localToWorldMatrix.MultiplyPoint3x4(cyllinderMidPoint);
            distance = 10.0f;
            Ray ray3D = new Ray(point, cyllinderMidPointWorld.ModifyY(point.y) - point);
            outRay = ray3D;
            pointOnCanvas = new Vector2(0.0f, 0.0f);
            if (Physics.Raycast(ray3D, out hit, float.PositiveInfinity, GetRaycastLayerMask()))
            {
                ...
                pointOnCanvas = (myCanvas.transform.localToWorldMatrix.MultiplyPoint3x4(pointOnCanvas));

                distance = (hit.point - point).magnitude;
                return true;
            }
