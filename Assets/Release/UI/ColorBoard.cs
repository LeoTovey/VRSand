...
public class ColorBoard : MonoBehaviour, IPointerClickHandler
{
    ...
    {
        Vector2 newPosition = 0.5f * ColorBoardTransform.rect.size + (eventData.position - (Vector2)ColorBoardTransform.position);
        ColorHandleTransform.anchoredPosition = GetClampPosition(newPosition);
        Color color = GetColorByPosition(ColorHandleTransform.anchoredPosition);
        ColorHandleImage.color = color;
    }
